...
assert.equal(next.pan, 0.08);
assert.equal(next.tilt, 0.2);
const lagging = {pan: 0.2, tilt: 0};
const chase = {cx: 0.9, cy: 0.42};
const firstChase = tracker.nextTarget(lagging, chase, {panSign: 1, tiltSign: -1}, {pan: 0.0825, tilt: 0.2});
const repeatedChase = tracker.nextTarget(lagging, chase, {panSign: 1, tiltSign: -1}, {pan: 0.0825, tilt: 0.2});
assert.equal(firstChase.pan, repeatedChase.pan); // No target wind-up while PWM is lagging.
assert.ok(firstChase.pan < 0.3);
...
assert.deepEqual(tracker.headAim({pan: 0.3, tilt: 0}, true), {pan: 'left', tilt: 'level'});
assert.equal(tracker.quickLookPosition('look right', true), -0.3);
assert.equal(tracker.quickLookPosition('look left', true), 0.3);
assert.throws(() => tracker.quickLookPosition('look right', null));
